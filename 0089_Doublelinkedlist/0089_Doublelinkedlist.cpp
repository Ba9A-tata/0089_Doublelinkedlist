...
void addNode()
{
	...
	cin >> newNode->name; // Assign value to the data field od the new node
	...
	// if the list is empty, amke the new node the START
	...
	else
	{
		...
		if (previous != NULL)
		{
			previous->next = newNode; // step 7: Make the field of the previous node point to the new node.
		}
		else
		{
...
void deleteNode()
{
	...
	if (current == NULL)
	{
		cout << "\033[31mThe record with the roll number " << rollNo << " not found\033[0m" << endl;
		return;
	}
	...
	{	// Node to be deleted is not the fisrt node
...
void searchData()
{
	...
	Node *prev, *curr;
	prev = curr = NULL;
	cout << "\nENter the roll number of the student whose record you want to search:"; int num;
	cin >> num;
...
int main()
{
	while (true)
	{
		try
		{
			...
			cout << "4. View all records in the asscending order of roll numbers" << endl;
